...
This function calls the method \verb@desc@ of the class \verb@Idea@, which provides a short description for the idea, taking care of the special case when the idea selected is ``void''.
\index{idea!desc}
<<>>=
Idea$methods(
             desc = function() {
...
\section{Replication}
The goal here is to find the range of parameters for which simulation runs of the model yield output that is similar to a target provided.

Birattari


Function that returns contingency table for licence type / status
<<>>=
wrap.table <- function(output) {
